...
def plot_control_correlation_heatmap(
    ...
    """Create correlation heatmap for control samples (reference, pool).
...
def plot_cv_distribution(
    ...
    Args:
        ...
        abundance_col: Column name with abundance values
        sample_type_col: Column name with sample type
        control_types: List of sample types to analyze (default: ['reference', 'pool'])
...
def plot_rt_residuals(
    ...
    """Plot abundance residuals vs retention time for reference and pool samples.
    ...
    Args:
        ...
        pool_type: Sample type value for pool/QC samples
        title: Plot title
    ...
    # Filter to reference and pool samples
    ref_samples = merged.loc[
        merged[sample_type_col] == reference_type, sample_col
    ].unique()
    pool_samples = merged.loc[
        merged[sample_type_col] == pool_type, sample_col
    ].unique()

    # Select subset of samples to show
    ref_to_show = ref_samples[: min(n_samples_to_show, len(ref_samples))]
    pool_to_show = pool_samples[: min(n_samples_to_show, len(pool_samples))]

    n_ref = len(ref_to_show)
    n_pool = len(pool_to_show)
    n_cols = max(n_ref, n_pool)

    if n_cols == 0:
        logger.warning("No reference or pool samples found")
        return None, merged
    ...
    pool_color = "#ff7f0e"  # Orange for pool
    ...
    # Plot pool samples (bottom row)
    for i in range(n_cols):
        ax = axes[1, i]
        if i < n_pool:
            plot_sample_residuals(ax, pool_to_show[i], pool_color, "Pool")
        else:
            ax.axis("off")

    axes[1, 0].set_ylabel("Pool Samples (QC)\nLog2 Residual", fontsize=10)
...
def plot_rt_correction_comparison(
    ...
    - Bottom row: Pool samples (held-out QC) before/after

    Reference samples should show improvement (used for fitting).
    Pool samples show how well the correction generalizes to held-out data.
    ...
    # Pool samples (held-out QC)
    pool_mad_before, pool_bias_before = plot_aggregated_residuals(
        axes[1, 0], residuals_before, pool_type, "#ff7f0e", "Pool (QC) - Before RT Correction"
    )
    pool_mad_after, pool_bias_after = plot_aggregated_residuals(
        axes[1, 1], residuals_after, pool_type, "#ff7f0e", "Pool (QC) - After RT Correction"
    )

    # Calculate improvements
    ref_mad_improvement = (ref_mad_before - ref_mad_after) / ref_mad_before * 100 if ref_mad_before > 0 else 0
    pool_mad_improvement = (pool_mad_before - pool_mad_after) / pool_mad_before * 100 if pool_mad_before > 0 else 0
    ref_bias_improvement = (ref_bias_before - ref_bias_after) / ref_bias_before * 100 if ref_bias_before > 0 else 0
    pool_bias_improvement = (pool_bias_before - pool_bias_after) / pool_bias_before * 100 if pool_bias_before > 0 else 0

    fig.suptitle(
        f"RT-Dependent Normalization QC\n"
        f"Reference: MAD {ref_mad_improvement:+.1f}%, RT bias {ref_bias_improvement:+.1f}% | "
        f"Pool (held-out): MAD {pool_mad_improvement:+.1f}%, RT bias {pool_bias_improvement:+.1f}%",
        fontsize=12,
        fontweight="bold",
        y=1.02,
    )
    ...
    logger.info(f"  Pool MAD: {pool_mad_before:.3f} → {pool_mad_after:.3f} ({pool_mad_improvement:+.1f}%)")
    logger.info(f"  Pool RT bias: {pool_bias_before:.3f} → {pool_bias_after:.3f} ({pool_bias_improvement:+.1f}%)")
...
def plot_rt_correction_per_sample(
    ...
    Creates a grid showing before/after for selected reference and pool samples.
    ...
    pool_samples = residuals_before.loc[
        residuals_before[sample_type_col] == pool_type, sample_col
    ].unique()[:n_samples]

    all_samples = list(ref_samples) + list(pool_samples)
    sample_types_list = [reference_type] * len(ref_samples) + [pool_type] * len(pool_samples)
    ...
    colors = {reference_type: "#2ca02c", pool_type: "#ff7f0e"}
    ...
    for row_idx, (sample_name, sample_type) in enumerate(zip(all_samples, sample_types_list)):
        ...
        for col_idx, (df, stage_label) in enumerate([
            ...
            type_label = "Ref" if sample_type == reference_type else "Pool"
