...
| *(not a config key)* | - | **The acquired denominator** comes from the instrument data files and is populated separately, by `prism ms2-signal -d <output-dir> -r <raw-dir>`. It writes `ms2_signal.parquet`, which `prism qc -d` joins onto the accounting at render time - so the plot gains an acquired bar and reports assigned/acquired. Kept out of `prism run` and out of the config because of the cost: a cohort is hundreds of gigabytes of raw data, usually over a share, against a pipeline that otherwise reads one exported report. Without it every number in the section is still correct; the plot simply shows assigned signal and the caption says so rather than implying a fraction |
