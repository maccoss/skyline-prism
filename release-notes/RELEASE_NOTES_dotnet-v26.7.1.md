# Skyline-PRISM (C#) dotnet-v26.7.1 Release Notes

Follow-up to 26.7.0, from using the new multi-document workflow on real data. Two Settings-tab controls
described the pipeline incorrectly - one named a shared-peptide policy that was not being applied and
could not be changed, the other implied a filter that discards proteins. Batch estimation now shows its
working, and warns in the case where it is most likely to invent batches. The intensity-distribution
plot is coloured the same way in the HTML report and in the tool. Exporting several documents now runs
them in parallel.
...
- The build is warning-free again: `Box.Fill` is obsolete in ScottPlot 5 (the RT-bin boxplot now sets
  `FillColor` directly), and a nullable-sender mismatch on the unobserved-task handler is fixed.
- The release workflow moved from `softprops/action-gh-release@v2` to `@v3`, which targets the Node 24
  Actions runtime; v2 declared Node 20 and the runners had begun warning about it. The action's inputs
  are unchanged.
