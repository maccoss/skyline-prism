# Skyline-PRISM (C#) dotnet-v26.10.0 Release Notes

The Dynamic Range plot is now genuinely interactive: selection works in both directions with the Skyline
document, its labels no longer stack on top of each other, and the two things that stopped it working at
all - clicking a protein doing nothing, and the peptide level failing outright - are fixed. Reading a
DIA acquisition's real isolation windows also works again, and can no longer delay a run.

Reference-anchored ComBat is a tick box in the tool, is the same estimator as the standard path (so
26.9.0's fixes apply to it too), and no longer forces the pipeline back into memory. Stage 1 now sizes
its memory from what is actually free.

> **Read the Breaking Changes section**: batch estimation from acquisition-time gaps is now off by
> default, and reference-anchored results move where a feature is constant within a batch.
...
- **Reading a DIA acquisition's real isolation windows works again, and can no longer hold up a run.**
  ...
  over named pipes). That path hangs on `--new`, which is exactly what this probe uses: it works
  against a throwaway document so your own is never touched. `SkylineDailyRunner.exe --new=x.sky
  --overwrite --save` prints `File x.sky opened.` and then nothing, never writing the file, while
  `SkylineCmd.exe` runs the identical arguments in **0.9 s**. The probe now uses `SkylineCmd`, which
  returns all 167 windows in **8.7 s**.

  This looks like a Skyline-daily issue rather than a PRISM one - it reproduces with MacCoss Lab's own
  `SkylineDailyRunner.exe`, no PRISM code involved - and has been raised with them. **Report export is
  unaffected**: it opens an existing document (`--in`), which the same runner handles in 1.6 s, so
  runs that add closed `.sky` documents still get parquet exactly as before.
  ...
  either**. The wait is now bounded, genuinely cancellable, and kills the Skyline it started when it
  gives up. `PRISM_ISOLATION_TIMEOUT_SEC` raises the 5-minute limit if your data really is that slow to
  reach.

  And it no longer runs **before** the pipeline. It is an enrichment for one plot - without it the
  density map falls back to clearly-labelled uniform bins and nothing else about the run changes - so it
  now runs alongside the run and writes its catalog when it finishes. The map reads that file when the
  tab is opened, so landing late costs nothing.
