# Skyline-PRISM (C#) dotnet-v26.9.0 Release Notes

Feature release adding two plot tabs to the Skyline tool - **Dynamic Range** and **Spectrum density** -
and a **Stop** button, so a run can be abandoned instead of waited out. Peptide normalization and ComBat
no longer hold the whole matrix in memory, which was the pipeline's memory wall on large cohorts, and the
merge no longer spills its sort onto a network drive.

It also fixes a serious ComBat bug: a single missing value turned every corrected value in the cohort into
NaN. ComBat is now validated against R's `sva::ComBat`, which it reproduces to floating-point noise on
dense input. **This changes protein-level results** where a feature is constant within a batch - see
"ComBat no longer invents batch effects it cannot estimate" below.
