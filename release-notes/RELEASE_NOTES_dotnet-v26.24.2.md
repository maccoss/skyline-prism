# Skyline-PRISM (C#) dotnet-v26.24.2 Release Notes

A one-fix release for anyone processing documents downloaded from PanoramaWeb: a `.sky.zip` no longer
extracts onto the share it came from, and no longer four at a time. On a real twelve-plate cohort that
is the difference between **216 MB/s** and ~15 MB/s - between minutes and most of a day.

Nothing else changed. If you are on 26.24.1 already this is only the extraction; from anything older it
carries all of 26.24.0 and 26.24.1 with it.
