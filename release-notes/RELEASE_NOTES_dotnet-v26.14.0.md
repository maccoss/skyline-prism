# Skyline-PRISM (C#) dotnet-v26.14.0

A correctness and coverage release. The peptide->protein median polish now writes its residuals, which
the Python engine has always produced and the C# engine silently did not - so an analysis built on that
file could not be reproduced on the C# engine at all. Both residual files are renamed to say which
output they explain, and CI now fails if any reported quantity changes by even one bit.
