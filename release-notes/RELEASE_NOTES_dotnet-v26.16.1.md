# Skyline-PRISM (C#) dotnet-v26.16.1 Release Notes

A correctness fix for the Spectrum density tab: an isolation scheme is now identified by its windows
rather than by its name, so a batch can no longer be binned on another batch's grid.
