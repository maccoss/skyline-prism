# Skyline-PRISM (C#) dotnet-v26.24.0 Release Notes

A new QC section answers "how much of the acquired MS2 signal does this analysis actually put a name
to?", and the Skyline tool now takes documents the way PanoramaWeb hands them out.

Getting there needed three things PRISM did not have: a way to count signal that two co-isolated
peptides share ONCE, the acquired total (which no Skyline export can supply, so the tool reads the
instrument files), and a measure that is the same quantity on both sides of the fraction. The section
is off by default - it costs an extra pass over the merged table - and every number in it is a raw
Skyline peak area, untouched by normalization or batch correction.
