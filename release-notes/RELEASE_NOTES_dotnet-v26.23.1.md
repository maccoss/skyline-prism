# Skyline-PRISM (C#) dotnet-v26.23.1 Release Notes

A bug-fix release, and the headline is not PRISM's bug. Skyline intermittently crashes about a second
into a headless launch, inside ClickOnce's activation setup and before any Skyline code runs, so there
is no window, no dialog and nothing in Skyline's log. PRISM did not look for the launched process until
its three-minute startup window expired, so a one-second crash cost three minutes, was reported as a
machine-load problem, and - because the parquet retries are budgeted in attempts rather than seconds -
exhausted the retry budget and fell back to CSV. All three consequences are fixed below. The Skyline-side
fault is reported upstream (support thread rowId 74874); PRISM's job is to notice it in seconds and retry.

Also in this release, with no change to any output: DuckDB.NET moved 1.5.3 -> 1.5.5 and Parquet.Net
5.6.1 -> 6.1.0. The second is a breaking API migration - v6 removes `DataColumn` - now isolated behind a
single `ParquetColumnIo` shim. Both were checked against a trusted run of a 192-sample cohort rather than
against the test suite alone: 45,972,678 numeric cells and 19 string columns compared, zero cells past
1e-9 relative, and the bit-exact quantity digests unchanged.
