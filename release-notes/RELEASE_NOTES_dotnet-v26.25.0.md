...
peptide of your analysis explains. Both sides are measured the same way from the same files, so the
ratio is a real fraction - which the measure it replaces never was, and which is why that one is
removed rather than kept alongside.

Around it: the tool window is split into **Analysis** and **Visualization**, PRISM **asks before
overwriting** a finished analysis rather than quietly replacing it, and a re-run now **reuses what
has not changed even when the previous run was on another computer** on the same file share.

The rest is what real use turned up - a density cell that counted the wrong thing, RT axes in the
wrong unit, the "Best" label inverted on small cohorts, an ion accounting file that lost a whole
measurement to an interrupted append, and a QC report that printed a paragraph under every figure.
...
- **A re-run from the tool reuses what has not changed, including across machines on a shared drive.**
  ...
  Two settings that are paths - `parsimony.fasta_path` and `transition_rollup.library_path` - still
  reach a stage fingerprint as absolute strings, so the protein and transition rollups can still
  differ between machines when a FASTA or a spectral library is configured. Everything above them
  agrees; those two stages recompute.
