# Skyline-PRISM (C#) dotnet-v26.19.0 Release Notes

Quantities can now be normalized to a set of marker proteins - asking what changed per unit of the
material a capture actually caught - and the Dynamic Range plot can be viewed under any protein rollup,
not just the one the run used. Two kidney marker panels ship alongside the EV one.
